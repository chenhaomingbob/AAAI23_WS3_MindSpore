...
class ActiveLearningSampler(ds.Sampler):
    ...
    def __init__(self, dataset, config, batch_size=6, split='training'):
        ...
        for tree in self.dataset.input_colors[split]:
    ...
    def spatially_regular_gen(self):
        ...
        for _ in range(self.n_samples * self.batch_size):  # num_per_epoch
            ...
            if self.split == 'training':
                s_indx = self.dataset.s_indx[self.split][cloud_idx]
                # Shuffle index
                queried_idx = np.concatenate([np.array(s_indx), queried_idx], 0)[:self.cfg.num_points]
...
def dataloader(config, **kwargs):
    dataset = S3DISDatasetGenerator(config.dataset_dir, config.labeled_percent)
